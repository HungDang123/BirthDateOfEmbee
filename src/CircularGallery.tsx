...
async function resolveFont(font, fontUrl) {
  ...
  if (!effectiveUrl) {
    if (document.fonts?.load) {
      try { await document.fonts.load(font); } catch { /* ignore */ }
    }
    return font;
  }
  try {
...
class Media {
  ...
  createShader() {
    const texture = new Texture(this.gl, { generateMipmaps: false, minFilter: this.gl.LINEAR, magFilter: this.gl.LINEAR });
    this.program = new Program(this.gl, {
      ...
      vertex: `
        ...
        void main() {
          vUv = uv;
          vec3 p = position;
          float amp = (0.1 + uSpeed * 0.5) * smoothstep(0.0, 0.5, abs(uSpeed));
...
class App {
  ...
  createMedias(items, bend, textColor, borderRadius, font) {
    const defaultItems = [
      { image: `https://picsum.photos/seed/1/800/600?grayscale`, text: 'Ảnh 1' },
      { image: `https://picsum.photos/seed/2/800/600?grayscale`, text: 'Ảnh 2' },
      { image: `https://picsum.photos/seed/3/800/600?grayscale`, text: 'Ảnh 3' },
    ];
...
export default function CircularGallery({ items, bend = 3, textColor = '#ffffff', borderRadius = 0.05, font = 'bold 30px Figtree', fontUrl, scrollSpeed = 2, scrollEase = 0.05 }) {
  ...
  return <div className="circular-gallery" ref={containerRef} tabIndex={0} role="region" aria-label="Circular image gallery" />;
}
